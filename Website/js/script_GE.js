...
async function carregarVagasGestor() {
  try {
    const resposta = await fetch(`http://98.95.103.3:5000/vagas/gestor/${id_usuario}`);
...
async function getVagaMetrics(vagaId) {
  try {
    const response = await fetch(`http://98.95.103.3:5000/processo-seletivo/vaga/${vagaId}`);
...
async function loadContent(page) {
  ...
  try {
    const resposta = await fetch(`http://98.95.103.3:5000/processo-seletivo/gestor/${id_usuario}`);
          ...
          ${candidatosTriagem.map(c => `
            <div class="card status-${c.status.toLowerCase().replace(/ /g, '-')}" data-status="${c.status}">
              ...
              <div class="action-icons">
                  ...
                  <img title="Excluir candidato" onclick="deletarCurriculo(${c.id_candidato})" class="icon-cards" src="/Website/img/lixo-icon.png" alt="">
              </div>
            </div>
          `).join("")}
        </div>
...
async function exibirCurriculo(id_candidato) {
  try {
    ...
    const resposta = await fetch(`http://98.95.103.3:5000/candidatos/${id_candidato}`);
...
async function listarCandidatosPorVaga(tituloVaga, idVaga) {
  ...
  try {
    ...
    const resposta = await fetch(`http://98.95.103.3:5000/processo-seletivo/vaga/${idVaga}`);
